...
class AdminJwtAuthenticationTest {
    ...
    void 유효한_어드민_JWT는_어드민_API를_통과한다() throws Exception {
        mockMvc.perform(get("/admin/probe")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + SecurityTestFixture.adminToken("admin")))
    ...
    void admin_스코프가_없는_JWT는_어드민_API에서_거부된다() throws Exception {
        mockMvc.perform(get("/admin/probe")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + SecurityTestFixture.adminToken("read")))
