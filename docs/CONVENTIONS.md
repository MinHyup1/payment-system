...
```bash
# 1. 이슈 생성 (번호를 받는다)
gh issue create --title "멱등키로 중복 승인 차단" --label "type/feat,phase/1,P01"
...
세 축으로 붙인다. **type은 필수**, 나머지는 해당될 때만.

| 축 | 라벨 | 용도 |
|---|---|---|
| **종류** | `type/feat` `type/fix` `type/docs` `type/test` `type/refactor` `type/perf` `type/ops` | 커밋 타입과 동일한 체계 |
| **단계** | `phase/0` ~ `phase/9` | 설계서 11장 로드맵 |
| **증명** | `P01` ~ `P10` | 설계서 05장의 문제 번호. **이 라벨이 붙은 이슈는 증명 테스트 없이 닫을 수 없다** |
| **상태** | `blocked` `needs-decision` | 진행 불가·판단 필요 |

`P01`~`P10` 라벨이 이 프로젝트의 핵심 장치다. 설계서가 "증명 칸이 비어 있으면 구현하지 않은 것으로 친다"고 선언했으므로, 라벨이 그 선언을 이슈 트래커에서 강제한다.
...
- [ ] `P0x` 라벨이 붙은 이슈라면, 그 문제를 증명하는 테스트가 이 PR에 있다
